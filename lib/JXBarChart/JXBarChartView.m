           ...
           gradient:(CGGradientRef)gradient
{
    ...
    if (self) {
        ...
        CGFloat colorComponents[] = {
            0.141, 0.722, 0.969, 1.0, //red, green, blue, alpha
            0.141, 0.722, 0.969, 1.0,
            0.141, 0.722, 0.969, 1.0
        };
//        [UIColor colorWithRed:0.141 green:0.722 blue:0.969 alpha:1]
        size_t count = 3;
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        NSLog(@"%@,%@",[self.values[i] stringValue],self.values[i]);
//        digitLabel.text = ![[self.values[i] stringValue] isEqualToString:@"0"]?[NSString stringWithFormat:@"%@0%%",[self.values[i] stringValue]]:[self.values[i] stringValue];
         digitLabel.text = ![[self.values[i] stringValue] isEqualToString:@"0"]?[NSString stringWithFormat:@"%@%%",[self.values[i] stringValue]]:[self.values[i] stringValue];
